...
void handle_list(mqd_t client_queues[MAX_CLIENTS]);
...
mqd_t server_qid;
int created_clients_count;
mqd_t client_queues[MAX_CLIENTS]; // position is client id at the same time
...
int main() {
	...
	Request request;
	while(1) {
		char *request_str = (char *)alloc(REQUEST_LEN);
		ssize_t res = mq_receive(
				server_qid,
				request_str,
				REQUEST_LEN,
				NULL);
		...
		if (res > 0) {
			request = to_request(request_str);
		}
...
void on_sigint(int _) {
	...
	char *resp_str = from_response(resp);
	...
	Request req;
	char *req_str = (char *)alloc(REQUEST_LEN);
...
void handle_init(
	...
	// get client queue id
	char *qpath = msg.mtext;
	mqd_t qid = open_queue(qpath);
	...
	char *resp_str = from_response(response);

	if(mq_send(qid, resp_str, RESPONSE_LEN, HIGH_PRIO) == -1) {
...
void handle_echo(mqd_t client_queues[MAX_CLIENTS], Request msg) {
...
void handle_to_friends(
	...
	printf("2FRIENDS %s\n", msg.mtext);
	mqd_t friends_qids[MAX_CLIENTS];
